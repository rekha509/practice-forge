## Current Phase: 10 — S10 rendering gaps fixed for real, verification infrastructure (calibration + blind re-solve) built and tested; blocked on a real, external time-gate (quota reset) for everything that needs to actually RUN
## Status: solutions manual now embeds real Python listings + real typeset math. Calibration mode (`pf calibrate`) and blind re-solve are real, tested, and ready — neither has been run against the real book yet. Blocked on gemini-flash-latest's real daily quota resetting (UTC, 2026-08-11 00:00) before: reverting the routing workaround, running calibration for real, re-verifying with blind resolve on the strong model, Course creation, ledger-commit, and the no-repeat proof. See the two most recent "Completed"/"Blocked On" sections below for the exact resume point. Task 2 (P10 API) is gated on a spec not yet received.
...
## Current build: P10 (API) — spec received, in progress. P11 (PWA) not started (explicit instruction: pass P10's gate first).

Full spec: FastAPI + Celery + Redis API, resumable chunked (tus-core-subset) upload for 2000-page books, real SSE progress with an ETA computed from observed pages/minute, generate/reshuffle/new-set as distinct actions (reshuffle never touches the ledger, new-set always does), section-scoped generation with a per-section cap that scales when few sections are selected, and a minimum-viable per-faculty bearer-token auth (user's explicit decision, see `docs/adr/0010`). **P10's gate** (`pytest tests/test_api.py` passes + a real upload-to-PDF run observed end to end through HTTP with SSE progress) **is NOT met yet** — `tests/test_api.py` does not exist, and no end-to-end run has happened. Everything below is real, written code, verified only at the level stated for each piece — no overclaiming.
...
**Verification level, stated precisely — this is the part that must not be overclaimed**: `ruff check` and `mypy src tests api worker` are both clean (confirmed twice, including after resuming this session — only the same 2 pre-existing lint / 6 pre-existing mypy errors remain, all in `test_scoring.py`/`test_concepts.py`/`test_detection.py`, none touched this work). `api.main.app` imports and constructs without error; `worker.tasks`'s four tasks register on `celery_app`. **None of that is the same as "it works."** No `tests/test_api.py` exists. No real HTTP request has ever hit any endpoint. No Celery worker has ever actually executed a task. The chunked-upload/SSE/generate/reshuffle/new-set flows have not been exercised even once, end to end or otherwise.

## Blocked On (current, real, environmental — not a code problem)
Resuming this session, **Docker Desktop is not starting** on this machine (`docker ps` fails: "Docker Desktop is unable to start" / "cannot find the pipe" on retry) — the dev Postgres and Redis containers this whole project's test suite and the API/worker both depend on are unreachable. This blocks: re-running the full `pytest` suite to reconfirm the 87/87-passing state from before this session's interruption, writing/running `tests/test_api.py`, and any real end-to-end HTTP+SSE run. Nothing about this is a code issue — `ruff`/`mypy` (which need no DB) are clean. **Resume point**: get Docker Desktop running again (restart it / the host, check for a stuck `com.docker.backend` process the way this project's earlier session-notes describe recovering from a similar hang), confirm `docker compose up -d db redis` succeeds, re-run the full suite to reconfirm nothing regressed, then write `tests/test_api.py` and run the real upload-to-PDF HTTP+SSE flow — that's what actually satisfies P10's gate. Only after that gate passes: start P11 (the PWA), per explicit instruction.
